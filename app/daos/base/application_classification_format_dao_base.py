...
class BaseApplicationClassificationFormatDao(BaseDao[ApplicationClassificationFormat]):
    ...
    def create(self, db_session: Session, data: Union[ApplicationClassificationFormat, dict]) -> ApplicationClassificationFormat:
        """
        ApplicationClassificationFormat を登録します。

        Args:
            db_session (Session): DBセッション
            data (ApplicationClassificationFormat or dict): 保存するインスタンスまたは dict
        ...
        try:
            if isinstance(data, dict):
                instance = ApplicationClassificationFormat(**data)
            else:
                instance = data
    ...
    def get_by_key(self, db_session: Session, id: int) -> List[ApplicationClassificationFormat]:
        """
        ApplicationClassificationFormat を指定された主キー条件で取得します。
    ...
    def get(self, db_session: Session, id: int) -> Optional[ApplicationClassificationFormat]:
        """
        主キーで単一取得。

        Returns:
            Optional[ApplicationClassificationFormat]: 該当するインスタンス or None
        """
    ...
    def update(self, db_session: Session, id: int, update_data: dict) -> Optional[ApplicationClassificationFormat]:
        ...
        Returns:
            Optional[ApplicationClassificationFormat]: 更新後インスタンス or None
        """
