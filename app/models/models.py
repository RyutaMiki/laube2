...
class ActivityTransit(Base):
    ...
    re_evaluation_condition = Column('re_evaluation_condition', String(255), nullable=True, comment="承認者の状態変化によって再評価される条件式（DSLやJSONLogicなど）")
    re_evaluation_mode = Column('re_evaluation_mode', EnumType(enum_class=ReEvaluationMode), nullable=True, comment="再評価時の遷移動作（SKIP / INSERT / OVERRIDE）")
...
class ReworkRoute(Base):
    ...
    rework_trigger_type = Column('rework_trigger_type', EnumType(enum_class=ReworkTriggerType), nullable=True, comment="差し戻しトリガーの種別（MANUAL / AUTO / TIMEOUT / POLICY）")
    is_dynamic_appendable = Column('is_dynamic_appendable', Boolean, nullable=False, default=False, comment="差し戻し先ルートに対して動的にステップを追加できるか")
    preserve_history_flag = Column('preserve_history_flag', Boolean, nullable=False, default=True, comment="差し戻し時に履歴を保持するか（Falseにすると初期化的に扱われる）")
