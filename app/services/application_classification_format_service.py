from typing import Optional
from sqlalchemy.orm import Session
from app.repositories.application_classification_format_repository import ApplicationClassificationFormatRepository
from app.models.models import ApplicationClassificationFormat
from app.services.base.base_service import BaseService


class ApplicationClassificationFormatService(BaseService):
    """
    ApplicationClassificationFormat に対応するサービスクラス。
    ビジネスロジックをここに記述。
    """
    def __init__(self, dao: Optional[ApplicationClassificationFormatRepository] = None):
        self.dao = dao or ApplicationClassificationFormatRepository()

    def get(self, db: Session, id: int) -> Optional[ApplicationClassificationFormat]:
        return self.dao.get(db, id)

    def get_all(self, db: Session, limit: int = 100, offset: int = 0):
        return self.dao.get_all(db, limit, offset)

    def create(self, db: Session, data: ApplicationClassificationFormat):
        return self.dao.create(db, data)

    def update(self, db: Session, instance: ApplicationClassificationFormat, values: dict):
        return self.dao.update(db, instance, values)

    def delete(self, db: Session, instance: ApplicationClassificationFormat):
        return self.dao.delete(db, instance)