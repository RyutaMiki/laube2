from typing import Optional
from sqlalchemy.orm import Session
from app.repositories.application_object_repository import ApplicationObjectRepository
from app.models.models import ApplicationObject
from app.services.base.base_service import BaseService


class ApplicationObjectService(BaseService):
    """
    ApplicationObject に対応するサービスクラス。
    ビジネスロジックをここに記述。
    """
    def __init__(self, dao: Optional[ApplicationObjectRepository] = None):
        self.dao = dao or ApplicationObjectRepository()

    def get(self, db: Session, id: int) -> Optional[ApplicationObject]:
        return self.dao.get(db, id)

    def get_all(self, db: Session, limit: int = 100, offset: int = 0):
        return self.dao.get_all(db, limit, offset)

    def create(self, db: Session, data: ApplicationObject):
        return self.dao.create(db, data)

    def update(self, db: Session, instance: ApplicationObject, values: dict):
        return self.dao.update(db, instance, values)

    def delete(self, db: Session, instance: ApplicationObject):
        return self.dao.delete(db, instance)