...
def test_create_and_get_user_group(db_session: Session, user_group_dict):
    dao = UserGroupDao()
    obj = dao.create(db_session, user_group_dict)
    found = dao.get(db_session, obj.id)
    assert found is not None

def test_update_user_group(db_session: Session, user_group_dict):
    dao = UserGroupDao()
    obj = dao.create(db_session, user_group_dict)
...
def test_delete_user_group(db_session: Session, user_group_dict):
    dao = UserGroupDao()
    obj = dao.create(db_session, user_group_dict)
