...
class BaseActivityObjectDao(BaseDao[ActivityObject]):
    ...
    def create(self, db_session: Session, data: Union[ActivityObject, dict]) -> ActivityObject:
        """
        ActivityObject を登録します。

        Args:
            db_session (Session): DBセッション
            data (ActivityObject or dict): 保存するインスタンスまたは dict
    ...
    def get_by_key(self, db_session: Session, id: int) -> List[ActivityObject]:
        """
        ActivityObject を指定された主キー条件で取得します。
    ...
    def get(self, db_session: Session, id: int) -> Optional[ActivityObject]:
        """
        主キーで単一取得。

        Returns:
            Optional[ActivityObject]: 該当するインスタンス or None
        """
    ...
    def update(self, db_session: Session, id: int, update_data: dict) -> Optional[ActivityObject]:
        ...
        Returns:
            Optional[ActivityObject]: 更新後インスタンス or None
        """
