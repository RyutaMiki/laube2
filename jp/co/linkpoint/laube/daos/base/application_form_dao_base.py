...
class BaseApplicationFormDao(BaseDao[ApplicationForm]):
    ...
    def create(self, db_session: Session, data: Union[ApplicationForm, dict]) -> ApplicationForm:
        """
        ApplicationForm を登録します。

        Args:
            db_session (Session): DBセッション
            data (ApplicationForm or dict): 保存するインスタンスまたは dict
    ...
    def get_by_key(self, db_session: Session, id: int) -> List[ApplicationForm]:
        """
        ApplicationForm を指定された主キー条件で取得します。
    ...
    def get(self, db_session: Session, id: int) -> Optional[ApplicationForm]:
        """
        主キーで単一取得。

        Returns:
            Optional[ApplicationForm]: 該当するインスタンス or None
        """
    ...
    def update(self, db_session: Session, id: int, update_data: dict) -> Optional[ApplicationForm]:
        ...
        Returns:
            Optional[ApplicationForm]: 更新後インスタンス or None
        """
